...
fn read_addon(path: &Path) -> Result<(PathBuf, PathBuf)> {
    ...
    let addon: Addon = serde_json::from_slice(&std::fs::read(path)?)?;
        ...
        addon.schema_version == 1
            && addon.worker_protocol == crate::engines::native_chromium::WORKER_PROTOCOL
...
pub fn open(app: &AlcoveApplication, config: &AppConfigV3, background: bool) -> Result<()> {
    ...
    let window = crate::engines::native_chromium::open(
        app.upcast_ref::<adw::Application>(),
        crate::engines::native_chromium::Launch {
...
impl crate::engines::native_chromium::PolicyStore for RepositoryPolicy {
    fn permissions(
        &self,
        origin: &Origin,
