...
fn backup_options() -> (
    ...
    let mismatch = crate::dialogs::inline_error();
...
pub fn start_backup(parent: &AlcoveWindow) {
    ...
    glib::spawn_future_local(async move {
        ...
        let progress = crate::dialogs::show_progress(&window, &gettext("Creating Backup…"));
...
pub fn start_restore(parent: &AlcoveWindow) {
    ...
    glib::spawn_future_local(async move {
        ...
        let plan = loop {
            ...
            let progress = crate::dialogs::show_progress(&window, &gettext("Opening Backup…"));
...
fn restore_passphrase_dialog(error: Option<&str>) -> (adw::AlertDialog, gtk::PasswordEntry) {
    ...
    let feedback = crate::dialogs::inline_error();
...
type RestoreSelection = std::rc::Rc<std::cell::RefCell<HashSet<crate::model::AppId>>>;

fn restore_options(
    entries: &[crate::backup::RestorePreviewEntry],
    description: &str,
) -> (crate::dialogs::ActionDialog, RestoreSelection) {
    let action_dialog =
        crate::dialogs::ActionDialog::new(&gettext("Restore Applications"), &gettext("Restore"));
    let page = adw::PreferencesPage::new();
    let group = adw::PreferencesGroup::builder()
...
async fn show_restore_preview(parent: &AlcoveWindow, plan: RestorePlan) {
    ...
    let progress = crate::dialogs::show_progress(parent, &gettext("Restoring Applications…"));
...
pub(crate) fn run_ui_smoke_test<P: IsA<gtk::Application>>(application: &P) -> anyhow::Result<()> {
    ...
    crate::ui_test_support::capture(&window, "backup-settings", 800, 760)?;
    ...
    crate::ui_test_support::capture(&window, "backup-invalid-narrow", 360, 640)?;
    confirm.set_text("test passphrase");
    ensure!(
        options.commit.is_sensitive(),
        "matching passphrases rejected"
    );
    crate::ui_test_support::capture(&window, "backup", 800, 760)?;
    crate::ui_test_support::capture(&window, "backup-narrow", 360, 640)?;
    adw::StyleManager::default().set_color_scheme(adw::ColorScheme::ForceDark);
    crate::ui_test_support::capture(&window, "backup-dark", 800, 760)?;
    ...
    .into_iter()
    .map(|(title, disposition)| crate::backup::RestorePreviewEntry {
        source_id: crate::model::AppId::generate(),
        target_id: crate::model::AppId::generate(),
        title: title.into(),
        disposition,
    })
    .collect::<Vec<_>>();
    ...
    crate::ui_test_support::capture(&window, "restore", 800, 760)?;
    crate::ui_test_support::capture(&window, "restore-narrow", 360, 640)?;
    adw::StyleManager::default().set_color_scheme(adw::ColorScheme::ForceDark);
    crate::ui_test_support::capture(&window, "restore-dark", 800, 760)?;
    adw::StyleManager::default().set_color_scheme(adw::ColorScheme::ForceLight);
    restore.dialog.force_close();
    crate::ui_test_support::settle();
    let progress = crate::dialogs::show_progress(&window, &gettext("Creating Backup…"));
    crate::ui_test_support::capture(&window, "backup-progress-narrow", 360, 640)?;
    ...
    crate::ui_test_support::capture(&window, "restore-passphrase-narrow", 360, 640)?;
