...
impl AppSetting {
    pub fn apply(&self, app: &mut AppConfigV3) {
        match self {
            Self::Title(value) => app.title.clone_from(value),
            Self::StartUrl(value) => app.start_url.clone_from(value),
            Self::ThemeColor(value) => app.use_theme_color = *value,
            Self::Engine(value) => app.engine = *value,
            Self::UserAgent(value) => app.user_agent.clone_from(value),
            Self::Icon(_) => {}
        }
    }
    ...
    pub fn requires_restart(&self) -> bool {
        matches!(
            self,
            Self::StartUrl(_) | Self::ThemeColor(_) | Self::Engine(_) | Self::UserAgent(_)
        )
    }
}
...
impl<L: LauncherBackend, B: BackgroundBackend, C: ChromiumBackend> AppService<L, B, C> {
    ...
    pub fn save_runtime_state(&self, id: &AppId, window: WindowState) -> Result<()> {
        self.repository
            .edit(id, None, |current| current.window = window)?;
        Ok(())
    }
    ...
    pub async fn change_setting(
        ...
        let previous = self.repository.load(id)?;
        let mut desired = previous.clone();
        setting.apply(&mut desired);
        desired.normalize_and_validate()?;
        let icon = match setting {
            AppSetting::Icon(bytes) => Some(bytes.as_slice()),
            _ => None,
        };
        if desired == previous && icon.is_none() {
            return Ok(previous);
        }
        // Only the name and icon are present in the desktop launcher. Changing
        // a runtime preference must not request another launcher installation.
        let launcher_changed = desired.title != previous.title || icon.is_some();
        let old_icon = if launcher_changed {
            let old_icon = self.repository.read_icon(id)?;
            self.launcher
                .install(&desired, icon.unwrap_or(&old_icon), parent)
                .await?;
            Some(old_icon)
        } else {
            None
        ...
        match self
            .repository
            .edit(id, icon, |current| setting.apply(current))
        {
            Ok(saved) => Ok(saved),
            Err(error) => {
                if let Some(old_icon) = old_icon {
                    // Do not roll back unrelated fields or newer window state.
                    if icon.is_some() {
                        self.repository
                            .edit(id, Some(&old_icon), |_| {})
                            .context("could not restore the previous icon")?;
                    }
                    self.launcher
                        .install(&previous, &old_icon, parent)
                        .await
                        .context("could not restore the previous launcher")?;
                }
                Err(error)
            }
        }
    ...
    pub async fn delete(&self, id: &AppId) -> Result<UninstallOutcome> {
        ...
        let result = async {
            ...
            // Removing the profile directory takes the engine's storage with
            // it, so there is nothing left for the engine to clean up.
            self.repository
...
mod tests {
    ...
    use crate::{
        domain::model::Engine,
        domain::policy::{Origin, PermissionDecision, PermissionKind},
        system::background::BackgroundGrant,
    };
    ...
    struct FakeLauncher {
        ...
        runtime_change_on_install: Rc<RefCell<Option<(AppRepository, AppId, WindowState)>>>,
    }
    ...
    impl LauncherBackend for FakeLauncher {
        async fn install(
            ...
            if let Some((repository, id, window)) =
                self.runtime_change_on_install.borrow_mut().take()
            {
                repository.edit(&id, None, |current| current.window = window)?;
            }
            Ok(())
        }
    ...
    #[test]
    fn individual_settings_preserve_runtime_state_without_reinstalling_launcher() {
        let (_temp, service, launcher) = service();
        let app = AppConfigV3::new("Example", "example.org", 0).unwrap();
        block_on(service.create(app.clone(), b"icon", None)).unwrap();
        *launcher.deny_install.borrow_mut() = true;
        ...
        block_on(service.change_setting(
            &app.id,
            &AppSetting::StartUrl("gnome.org".to_owned()),
            None,
        ))
        .unwrap();
        block_on(service.change_setting(&app.id, &AppSetting::ThemeColor(false), None)).unwrap();
        block_on(service.change_setting(
            &app.id,
            &AppSetting::UserAgent(Some("Test Agent".to_owned())),
            None,
        ))
        .unwrap();
        let saved =
            block_on(service.change_setting(&app.id, &AppSetting::Engine(Engine::Chromium), None))
                .unwrap();
        assert_eq!(saved.window, window);
        assert_eq!(saved.title, app.title);
        assert_eq!(saved.start_url, "https://gnome.org/");
        assert!(!saved.use_theme_color);
        assert_eq!(saved.user_agent.as_deref(), Some("Test Agent"));
        assert_eq!(saved.engine, Engine::Chromium);
        service
            .save_runtime_state(&app.id, WindowState::default())
            .unwrap();
        assert_eq!(service.load(&app.id).unwrap().user_agent, saved.user_agent);
    }

    #[test]
    fn rejected_identity_edit_keeps_saved_settings_and_icon() {
        let (_temp, service, launcher) = service();
        let app = AppConfigV3::new("Example", "example.org", 0).unwrap();
        block_on(service.create(app.clone(), b"icon", None)).unwrap();
        *launcher.deny_install.borrow_mut() = true;
        for edit in [
            AppSetting::Title("Renamed".to_owned()),
            AppSetting::Icon(b"new icon".to_vec()),
        ] {
            assert!(block_on(service.change_setting(&app.id, &edit, None)).is_err());
            assert_eq!(service.load(&app.id).unwrap(), app);
            assert_eq!(service.read_icon(&app.id).unwrap(), b"icon");
        }
        // A no-op does not ask for confirmation either.
        assert!(block_on(service.change_setting(
            &app.id,
            &AppSetting::Title(app.title.clone()),
            None
        ))
        .is_ok());
        *launcher.deny_install.borrow_mut() = false;
        let saved = block_on(service.change_setting(
            &app.id,
            &AppSetting::Title("Renamed".to_owned()),
            None,
        ))
    ...
    #[test]
    fn rename_keeps_window_changes_made_during_portal_confirmation() {
        let (_temp, service, launcher) = service();
        let app = AppConfigV3::new("Example", "example.org", 0).unwrap();
        block_on(service.create(app.clone(), b"icon", None)).unwrap();
        let window = WindowState {
        ...
        launcher.runtime_change_on_install.replace(Some((
        ...
        assert_eq!(saved.title, "Renamed");
        assert_eq!(saved.window, window);
        assert_eq!(service.load(&app.id).unwrap(), saved);
    }
