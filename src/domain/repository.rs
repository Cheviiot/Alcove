...
impl AppRepository {
    ...
    pub fn update(&self, app: &AppConfigV3, icon: Option<&[u8]>) -> Result<()> {
        ...
        self.write_update_locked(app, icon)
    }

    /// Apply a field edit to the latest record while holding its metadata lock.
    /// Runtime window changes and independent edits must survive autosaving.
    pub fn edit(
        &self,
        id: &AppId,
        icon: Option<&[u8]>,
        apply: impl FnOnce(&mut AppConfigV3),
    ) -> Result<AppConfigV3> {
        let _metadata_lock = self.lock_app_file(id, METADATA_LOCK_FILE)?;
        let mut current = self.load_from_path_locked(&self.app_dir(id).join(CONFIG_FILE))?;
        apply(&mut current);
        current.normalize_and_validate()?;
        self.write_update_locked(&current, icon)?;
        Ok(current)
    }

    fn write_update_locked(&self, app: &AppConfigV3, icon: Option<&[u8]>) -> Result<()> {
        let app_dir = self.app_dir(&app.id);
...
mod tests {
    use super::*;
    use crate::{
        domain::model::SCHEMA_VERSION,
        domain::policy::{Origin, PermissionDecision, PermissionKind, POLICY_SCHEMA_VERSION},
    };
    use std::str::FromStr;
  ...
  "sort_order": 1
        ...
        let app = repository.load(&id).unwrap();
        assert_eq!(app.schema_version, SCHEMA_VERSION);
        assert_eq!(app.engine, crate::domain::model::Engine::WebKit);
        assert_eq!(
            fs::read(profile_dir.join("profile-state")).unwrap(),
            b"webkit profile"
        );
        ...
        repository
            .mutate_policy(&app.id, |policy| {
                policy.content_filters.insert(
                    ...
                    crate::domain::policy::ContentFilterRuleSet::new(
                        "Removed by editor",
                        serde_json::json!([]),
                    )
                    .unwrap(),
