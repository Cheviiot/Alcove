        ...
        fn command_line(&self, command_line: &gio::ApplicationCommandLine) -> glib::ExitCode {
            ...
            if library_only
                ...
                || command_line
                    ...
                    .unwrap_or(false)
            {
                crate::ui_test_support::prepare();
                let result = if library_only {
                    crate::window::run_ui_smoke_test(&*self.obj(), true)
                } else if creation_only {
                    crate::create_app_dialog::run_ui_smoke_test(&*self.obj())
                } else if policy_only {
                    crate::permissions_dialog::run_ui_smoke_test(&*self.obj())
                        .and_then(|()| crate::privacy_dialog::run_ui_smoke_test(&*self.obj()))
                } else if settings_only {
                    crate::app_page::run_ui_smoke_test()
                        .and_then(|()| crate::window::render_settings(&*self.obj()))
                } else if utilities_only {
                    crate::backup_dialog::run_ui_smoke_test(&*self.obj())
                        .and_then(|()| crate::window::render_utilities(&*self.obj()))
                    ...
                    crate::app_page::run_ui_smoke_test()
                        ...
                        .and_then(|()| crate::privacy_dialog::run_ui_smoke_test(&*self.obj()))
                        .and_then(|()| crate::permissions_dialog::run_ui_smoke_test(&*self.obj()))
                        .and_then(|()| crate::window::run_ui_smoke_test(&*self.obj(), false))
                        .and_then(|()| crate::create_app_dialog::run_ui_smoke_test(&*self.obj()))
                        .and_then(|()| {
                            crate::app_window::run_background_ui_smoke_test(&*self.obj())
                        })
                };
            ...
            if let Some(id) = app_id {
                ...
                if let Some(window) =
                    crate::native_chromium_launch::existing_window(&self.obj(), &id)
                {
                    if !start_in_background {
                ...
                match config.engine {
                    ...
                    Engine::Chromium => {
                        #[cfg(feature = "native-chromium")]
                        let result = if crate::native_chromium_launch::enabled() {
                            crate::native_chromium_launch::open(
                                &self.obj(),
                                &config,
                                start_in_background,
                            )
        ...
        self.add_action_entries([
            ...
            gio::ActionEntry::builder("show-background")
                ...
                .activate(|app: &Self, _, parameter| {
                    ...
                    #[cfg(feature = "native-chromium")]
                    crate::native_chromium_launch::background_action(
                        app,
                        parameter,
                        "win.show-background",
                    );
