...
use crate::{
    domain::model::AppId,
    domain::policy::{PermissionDecision, PermissionKind},
...
pub(super) fn present_editor(
    parent: &gtk::Window,
    id: AppId,
    policy: crate::domain::policy::AppPolicyV2,
) -> adw::Dialog {
    ...
    for (origin, permissions) in policy.permissions {
        ...
        for (kind, decision) in permissions {
            ...
            let row = adw::ComboRow::builder()
                ...
                .build();
            let error = crate::ui::common::inline_error();
                ...
                move |row| {
                        ...
                        .apply_policy_decisions(&id, &[(origin.clone(), kind, requested)])
                    {
                        ...
                        Err(failure) => {
                            ...
                            resetting.set(false);
                            crate::ui::common::show_inline_error(&error, &failure.to_string());
                        }
                    }
                }
            ));
    ...
    if rows.borrow().is_empty() {
        ...
        reset.connect_activated(glib::clone!(
            ...
            move |button| match AppService::portal().reset_policy(&id) {
                ...
                Err(failure) => crate::ui::common::show_inline_error(&error, &failure.to_string()),
            }
        ));
    }
    dialog.add(&page);
