...
use super::store::{permission_kinds, Policy};
use crate::domain::policy::{Origin, PermissionDecision, PermissionKind};
use crate::ui::shell::chromium::{site::files, widgets::chrome::Chrome, worker::process::View};
use adw::prelude::*;
        ...
        if matches!(kind, "file-dialog" | "download-request") {
            ...
            *self.active.borrow_mut() = Some(Active {
            ...
            glib::spawn_future_local(async move {
                let result = files::choose(&window, &event, cancel).await;
