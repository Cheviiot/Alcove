...
impl State {
    ...
    fn update(&mut self, next: State) -> Result<bool> {
        let limit = crate::engines::chromium::protocol::MAX_DIMENSION as i32;
        ensure!(
            next.generation > 0
...
pub struct PopupSurface {
    pub layer: Layer,
    state: State,
    gpu: Option<crate::ui::shell::chromium::worker::gpu::Texture>,
    cpu: Option<u64>,
...
impl PopupSurface {
    ...
    pub fn queue_gpu(&mut self, frame: crate::ui::shell::chromium::worker::gpu::Texture) {
    ...
    pub fn present(&mut self, directory: &Path, scale: i32) -> Result<Option<u64>> {
            ...
            .is_some_and(|generation| self.state.accepts(generation))
        {
            ...
            match crate::ui::shell::chromium::worker::frame::cpu_texture(&path) {
