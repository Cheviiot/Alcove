...
use crate::{
    domain::model::{AppConfigV3, Engine},
    ...
    impl AppPage {
        ...
        async fn on_icon_clicked(&self, _row: adw::ActionRow) {
            ...
            let result = async {
                ...
                icons::normalize_icon(bytes.to_vec()).await
            }
            .await;
            page.set_busy(false);
            match result {
...
impl AppPage {
        ...
        glib::spawn_future_local(glib::clone!(
            ...
            async move {
                if let Ok(bytes) = AppService::portal().read_icon(&config.id) {
                    if let Ok(texture) = icons::load_texture(bytes).await {
    ...
    pub(super) fn populate(&self, config: &AppConfigV3) {
        let imp = self.imp();
            ...
            Ok(saved) => {
                ...
                match &setting {
                    ...
                    AppSetting::Icon(icon) => {
                        ...
                        if let Ok(texture) = icons::load_texture(icon.clone()).await {
