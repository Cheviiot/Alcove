...
fn present_editor(parent: &AlcoveWindow, policy: AppPolicyV2, config: AppConfigV3) -> adw::Dialog {
    ...
    let navigation_error = crate::dialogs::inline_error();
    ...
    let proxy_error = crate::dialogs::inline_error();
    ...
    let background_error = crate::dialogs::inline_error();
    ...
    let filter_error = crate::dialogs::inline_error();
                    ...
                    async move {
                        ...
                        match result {
                            ...
                            Err(error) if !is_cancelled(&error) => {
                                crate::dialogs::show_inline_error(
                                    &editor.filter_error,
                                    &error.to_string(),
                                )
                            }
...
fn apply_edit(policy: &mut AppPolicyV2, edit: &Edit, start_url: &str) -> Result<()> {
    match edit {
    ...
    let file = gtk::FileDialog::builder()
        ...
        .map_err(|error| {
            crate::portal::classify_file_dialog_error(gettext("Import content filter"), &error)
                ...
                .unwrap_or_else(|| {
                    crate::portal::PortalOperationError::new(
                        crate::portal::PortalFailureKind::Cancelled,
...
fn is_cancelled(error: &anyhow::Error) -> bool {
    error
        ...
        .is_some_and(|error| error.kind == crate::portal::PortalFailureKind::Cancelled)
}
...
pub(crate) fn run_ui_smoke_test<P: IsA<gtk::Application>>(application: &P) -> Result<()> {
    ...
    apply_edit(
        &mut policy,
        &Edit::Proxy(ProxyMode::Custom, "socks5://127.0.0.1:1080".into()),
        &config.start_url,
    )?;
    ...
    let dialog = present_editor(&parent, policy, config);
    crate::ui_test_support::capture(&parent, "privacy", 800, 760)?;
    crate::ui_test_support::capture(&parent, "privacy-narrow", 360, 640)?;
    ...
    crate::ui_test_support::capture(&parent, "privacy-dark", 800, 760)?;
...
mod tests {
    ...
    fn cancellation_is_not_inferred_from_a_file_error_message() {
        ...
        let cancelled = crate::portal::PortalOperationError::new(
            crate::portal::PortalFailureKind::Cancelled,
    ...
    fn field_edits_preserve_unrelated_policy_and_keep_background_opt_in() {
        ...
        policy.set_decision(
            origin.clone(),
            crate::policy::PermissionKind::Camera,
            crate::policy::PermissionDecision::Block,
        );
        apply_edit(
            &mut policy,
        ...
        .unwrap();
        assert_eq!(
            policy.decision(&origin, crate::policy::PermissionKind::Camera),
            crate::policy::PermissionDecision::Block
        );
