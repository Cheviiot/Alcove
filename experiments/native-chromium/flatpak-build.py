...
Run inside alcove-dev. The production manifest and ordinary installation stay
...
def manifest():
    ...
    native = {
        'name': 'alcove-native-chromium', 'buildsystem': 'simple',
        'build-commands': [
            'cmake -S . -B _build -G Ninja -DCEF_ROOT="$PWD/cef" -DCMAKE_BUILD_TYPE=Release',
            'cmake --build _build --parallel ${FLATPAK_BUILDER_N_JOBS}',
            f'install -Dm755 _build/alcove-cef-worker {PREFIX}/worker/alcove-cef-worker',
            ...
            f'install -Dm755 flatpak-worker.sh {PREFIX}/bin/alcove-cef-launch',
        ...
        'sources': [
            *[{'type': 'file', 'path': str(ROOT / 'experiments/native-chromium' / name)}
            ...
            {'type': 'inline', 'dest-filename': 'addon.json', 'contents': json.dumps({
                ...
                'worker': 'bin/alcove-cef-launch', 'cef_root': 'cef',
            })},
        ],
    }
    app = modules['alcove']
    ...
    app['post-install'] = [
        f'install -d {PREFIX}',
        'cargo build --offline --locked --release --features native-chromium-probe --bin alcove-native-chromium --manifest-path=/run/build/alcove/Cargo.toml --target-dir=/run/build/alcove/_flatpak_build/src',
        'install -Dm755 /run/build/alcove/_flatpak_build/src/release/alcove-native-chromium /app/bin/alcove-native-chromium',
        'install -Dm755 /run/build/alcove/experiments/native-chromium/flatpak-probe.sh /app/bin/alcove-native-probe',
    ]
    for module in (modules['alcove-chromium-broker'], app):
    ...
    base['finish-args'].append(f'--env=ALCOVE_NATIVE_CHROMIUM_ADDON={PREFIX}/addon.json')
    base['modules'] = [modules['blueprint-compiler'], modules['alcove-chromium-broker'],
        zypak, {
...
def main():
    ...
    if args.host_only:
        ...
        content['modules'] = [m for m in content['modules'] if m['name'] in
            ('blueprint-compiler', 'alcove-chromium-broker', 'alcove')]
