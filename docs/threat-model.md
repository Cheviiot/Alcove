...
This document covers Alcove's WebKit and Chromium runtimes, per-application
...
- Alcove's local repository and the Flatpak sandbox are trusted to enforce
...
- File Chooser and Dynamic Launcher Portal responses are user-authorized input.
  Alcove does not bypass a missing or denied portal by writing to the host.
- A `.alcove-backup` is untrusted until its archive structure, size limits,
...
- The Chromium engine is included in the Alcove Flatpak and therefore shares
...
- Prevent one Alcove application from accidentally reading or deleting another
...
| Unexpected top-level navigation | Optional origin allowlist with one-time, persistent, external-browser, or block choices. WebKit identifies the main resource at response-policy time, so a disallowed server may receive the initial request before Alcove prevents the response from becoming the top-level document. |
...
- Alcove does not promise DRM/Widevine, browser extensions, proprietary browser
  APIs, anti-bot bypasses, or behavior identical to Chromium.
- WebKit, the desktop portal backend, the network, and sites can fail
  independently. Alcove must retain editable local metadata and offer precise
...
- A proxy protects only WebKit network traffic configured for that application's
  `NetworkSession`; portal traffic and Alcove's metadata fetches are outside
...
single Alcove sandbox. Process and profile separation reduce accidental
